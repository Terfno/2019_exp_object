...
marray=>operation: クラスの人数+1の長さの配列を用意する

i=>operation: i=0
for=>condition: i<クラスの人数
f_y=>operation: 配列番号=出席番号となる場所に点数を格納
i+=>operation: i++

en=>end: 処理終了
用意された配列に出席番号順にソートされた点数が格納されている。

st->marray->i->for
for(yes,right)->f_y(right)->i+(right)->for
for(no)->en
```
